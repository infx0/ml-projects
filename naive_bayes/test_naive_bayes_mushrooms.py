from naive_bayes_mushrooms import (
    clean_data,
    probability_of,
    normalize,
    find_best,
...
def test_probability_of():
    mock_nbc = {
        "priors": {"e": 0.6666666666666666, "p": 0.3333333333333333},
        "conditionals": {
            "e": {
                "cap-shape": {"b": 0.4, "x": 0.2, "f": 0.4},
                "cap-surface": {"s": 0.4, "y": 0.4, "f": 0.2},
                "cap-color": {"e": 0.5, "y": 0.5},
            },
            "p": {
                "cap-shape": {"b": 0.25, "x": 0.5, "f": 0.25},
                "cap-surface": {"s": 0.25, "y": 0.25, "f": 0.5},
                "cap-color": {"e": 0.3333333333333333, "y": 0.6666666666666666},
            },
        },
    }
...
def test_classify():
    mock_training_data = [
        {"eat": "e", "cap-shape": "x", "cap-surface": "s", "cap-color": "y"},
    ...
    nbc = train(
        mock_training_data, ["cap-shape", "cap-surface", "cap-color"], smoothing=True
    )
    ...
    expected_nbc = {
        ...
        "conditionals": {
            "e": {
                "cap-shape": {"x": 0.5, "f": 0.5},
                "cap-surface": {"s": 0.5, "f": 0.5},
                "cap-color": {"e": 0.25, "y": 0.75},
            },
            "p": {
                "cap-shape": {"x": 0.6666666666666666, "f": 0.3333333333333333},
...
def test_classify_all():
    ...
    mock_test_instance_labeled = [
        {"eat": "e", "cap-shape": "f", "cap-surface": "s", "cap-color": "e"}
    ]
    mock_test_instance_unlabeled = [
        {"cap-shape": "f", "cap-surface": "s", "cap-color": "e"}
    ]

    nbc = train(
        mock_training_data, ["cap-shape", "cap-surface", "cap-color"], smoothing=True
    )
    results_labeled = classify_all(
        nbc=nbc, observations=mock_test_instance_labeled, labeled=True
    )
    results_unlabeled = classify_all(
        nbc=nbc, observations=mock_test_instance_unlabeled, labeled=False
    )
    expected_nbc = {
        "priors": {"e": 0.6666666666666666, "p": 0.3333333333333333},
        "conditionals": {
